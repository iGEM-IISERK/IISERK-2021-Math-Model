...
VidObj_live = VideoWriter("biofilm.avi");
open(VidObj_live);
max = 1; min = 0;


% VidObj_dead = VideoWriter("dead.avi");
% open(VidObj_dead);
...
% volfrac = figure;
% volfrac.Position = [100 100 550 400];
...
for counter = 0:loop_count
    ...
    %     if(rem(counter,frame_every_x_loops) == 0)
    %         quo = counter/frame_every_x_loops;
    %         filenamed = quo + ".png";
    ...
    sgtitle(sprintf(" Bacteria Volume Fraction at time = %d", counter*del_t))
    
    ax1 = subplot(1,2,1)
    contour(bact_live_vol_frac)
    set(gca,'Color','k');
    colormap(ax1, autumn); colorbar;
    caxis([min max]);
    title("Live Bacteria")
    
    ax2 = subplot(1,2,2)
    contour(bact_dead_vol_frac)
    %     contour(1- bact_live_vol_frac)
    set(gca,'Color','k');
    colormap(ax2, winter); colorbar; caxis([min max]);
    
    title("Dead Bacteria");
    
    %     subplot(1,2,2)
    %     contour(antimicrobe_con_mat)
    %     colorbar
    %     title("Dead Bacteria");
    ...
    %     mean_dead = [mean_dead, mean(nonzeros(1-bact_live_vol_frac), 'all')];
    ...
    %         figure
    %         contour(1 - bact_dead_vol_frac)
    %         colorbar; caxis([min max]);
    %         title("Live Bacteria Volume fraction")
    %         subtitle(sprintf("time = %d", counter))
    %         F2 = getframe(gcf);
    %         writeVideo(VidObj_dead,F2);   
...
%csvwrite("1bact.csv",bact_vol_frac);
%csvwrite("2sol.csv",sol_vol_frac);
%csvwrite("3live.csv",bact_live_vol_frac);
%csvwrite("4dead.csv",bact_dead_vol_frac);
...
% figure
% contour(in_d)
% title("Initial Dead Bacteria Volume fraction")
...
figure
hold on
plot(time_arr, mean_live, "LineWidth", 2.5)
plot(time_arr, mean_dead, "LineWidth", 2.5)
...
% Gradient
function [x_mat, y_mat] = gradient_fun(in_mat)
x_mat = par_x(in_mat);
y_mat = par_y(in_mat);
end
% Divergence
